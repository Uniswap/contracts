/**
 * Slipstream historical pool discovery.
 * Scans PoolCreated events from the Slipstream (Velodrome/Aerodrome CL) factory
 * and outputs pool configs in createPools.ts SlipstreamPoolConfig format.
 *
 * Usage:
 *   npx tsx historical/Slipstream.ts --chain-id 10
 *
 * Options:
 ...
 *   RPC_URL                 (required)
 *   SLIPSTREAM_FACTORY      (required — no chain-agnostic default; set SLIPSTREAM_FACTORY_<chainId>)
 *   RPS                     (optional, default 80)
 *
 * Output: JSON array in SlipstreamPoolConfig format.
 * Fee is always 0 (Slipstream pools key by tickSpacing, not fee).
 */
...
import { pRateLimit, ensureDirForFile } from '@src/utils';
import type { Address } from '../creation-modules/types.js';

const OUTPUT_FILE = 'slipstream-pools.json';
const ZERO_ADDRESS = '0x0000000000000000000000000000000000000000';

// Slipstream CLFactory emits tickSpacing (not fee) as the third indexed param
const FACTORY_ABI = [
...
async function getLogsWithRetry(
  ...
  try {
    ...
    if (!isRangeLimitError(err) || filter.toBlock <= filter.fromBlock)
      throw err;
...
async function main() {
  ...
  if (!factoryRaw) {
    console.error(
      'Missing env: SLIPSTREAM_FACTORY (or SLIPSTREAM_FACTORY_<chainId>)',
    );
    process.exit(1);
  }
  ...
  console.error(
    `Scanning blocks ${startBlock}..${endBlock} (chunk=${chunkSize})`,
  );

  const seen = new Set<Address>();
  const configs: SlipstreamPoolConfig[] = [];
  ...
  for (let from = startBlock; from <= endBlock; from += chunkSize) {
    ...
    for (const log of logs) {
      ...
      // Skip native currency — SlipstreamAggregator reverts on address(0)
      if (isNative(token0) || isNative(token1)) continue;
      if (seen.has(pool)) continue;
      seen.add(pool);

      // token0 < token1 guaranteed by the factory
      configs.push({
        poolType: 'slipstream',
        slipstreamPool: pool,
        currency0: token0,
    ...
    if (to === endBlock || (to - startBlock) % (chunkSize * 10n) === 0n) {
      console.error(
        `[scan] ${from}..${to} — found ${configs.length} pools so far`,
      );
    }
