/**
 * Uniswap V3 pool discovery (checkpoint-based polling).
 * Scans new PoolCreated events from the V3 factory since the last checkpoint
 * and appends new pools to the output file.
 *
 * Usage:
 *   npx tsx polling/UniswapV3.ts --chain-id 1
 *
 * Options:
...
async function main() {
  ...
  let allRecords = safeReadJson<UniswapV3PoolConfig[]>(outPath) ?? [];
  let totalLogs = 0;
  let newPools = 0;
