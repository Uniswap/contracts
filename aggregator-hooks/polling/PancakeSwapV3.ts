/**
 * PancakeSwap V3 pool discovery (checkpoint-based polling).
 * Scans new PoolCreated events from the PancakeSwap V3 factory since the last
 * checkpoint and appends new pools to the output file.
 *
 * Usage:
 *   npx tsx polling/PancakeSwapV3.ts --chain-id 1
 *
 * Options:
 ...
 *   RPC_URL                      (required)
 *   PANCAKESWAP_V3_FACTORY       (optional, default mainnet factory)
...
const OUTPUT_FILE = "pancakeswapv3-pools.json";
const CHECKPOINT_FILE = "pancakeswapv3_checkpoint.json";
...
function loadExistingKeys(outFile: string): Set<string> {
  ...
  try {
    const arr = safeReadJson<PancakeSwapV3PoolConfig[]>(outFile);
...
async function main() {
  ...
  const factoryRaw = getEnvForChain("PANCAKESWAP_V3_FACTORY", chainId) ?? DEFAULT_FACTORY;
  ...
  let allRecords = safeReadJson<PancakeSwapV3PoolConfig[]>(outPath) ?? [];
  let totalLogs = 0;
  let newPools = 0;
  ...
  for (let start = fromBlock; start <= toBlock; start += chunkBlocks) {
    ...
    const newRecords: PancakeSwapV3PoolConfig[] = [];
    ...
    for (const log of logs) {
      ...
      newPools++;
      newRecords.push({ poolType: "pancakeswapv3", v3Pool: pool, currency0: token0, currency1: token1, fee, tickSpacing, sqrtPriceX96: null });
    }
