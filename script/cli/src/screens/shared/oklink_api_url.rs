...
pub struct OklinkApiUrlScreen {
    text_input: TextInputComponent,
}

impl OklinkApiUrlScreen {
    pub fn new() -> Result<Self, Box<dyn std::error::Error>> {
        ...
        let env_var = format!("OKLINK_API_URL_{}", chain_id);
        let default_url = std::env::var(&env_var)
            .or_else(|_| std::env::var("OKLINK_API_URL"))
            .unwrap_or_default();

        Ok(OklinkApiUrlScreen {
            text_input: TextInputComponent::new(false, default_url, |input, _| input),
        })
    }
}

impl Screen for OklinkApiUrlScreen {
    fn render_content(&self, buffer: &mut Buffer) -> Result<(), Box<dyn std::error::Error>> {
        buffer.append_row_text("Enter the OKLink API URL:\n\n");
