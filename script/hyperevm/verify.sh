...
# `python3 script/hyperevm/check_reproducibility.py --pin` green (proves bytecode == source at the recorded settings).
#
...
J=deployments/json/$CHAIN.json; T=script/deploy/tasks/$CHAIN/task-pending.json
...
if [[ ${#ONLY[@]} -eq 0 || " ${ONLY[*]} " =~ " UniversalRouter " || " ${ONLY[*]} " =~ " PermissionsAdapterFactory " ]]; then
  pin src/pkgs/universal-router $BRIEFCASE_REGEN; pin src/pkgs/v4-periphery $BRIEFCASE_REGEN; forge build --skip '*/node_modules/*' >/dev/null
  ...
  URARGS=$(enc 'constructor((address,address,address,address,bytes32,bytes32,address,address,address,address,address))' \
    "($PERMIT2,$WETH,$V2F,$V3F,0x96e8ac4277198ff8b6f785478aa9a39f403cb768dd02cbee326c3e7da348845f,0xe34f199b19b2b4f47f68442619d555527d244f78a3297ea89325f843f87b8b54,$PM,$NPM,$POSM,$SPOKE,$PAF)")
  v UniversalRouter $UR src/pkgs/universal-router/contracts/UniversalRouter.sol:UniversalRouter 0.8.26 1 cancun yes "$URARGS"
  pin src/pkgs/universal-router HEAD; pin src/pkgs/v4-periphery HEAD
fi
